    ...
    <div className="flex flex-col h-full">
        ...
        {navItems.map(({ path, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2 rounded-lg w-full text-left ${
              isActive(path) ? "bg-blue-500 text-white" : "text-white/70 hover:bg-white/10"
            }`}
          >
            <Icon size={17} />
            {label}
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleLogout} className="text-white/40 hover:text-red-400 flex-shrink-0">
            <LogOut size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-slate-900/50 backdrop-blur-sm flex-shrink-0">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="capitalize text-sm text-white/50">{user?.role}</div>
