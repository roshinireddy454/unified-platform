...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => { socket.emit("join-user-room", user._id.toString()); });
  ...
  // Render the dropdown in a portal so it's above ALL components
  const dropdownPortal = open && createPortal(
    <div
      ...
      className="bg-slate-900/98 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]"
    >
      {/* Header */}
