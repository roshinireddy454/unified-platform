...
var CallRecordingFailedEventRecordingTypeEnum = {
  COMPOSITE: "composite",
  INDIVIDUAL: "individual",
  RAW: "raw"
};
var CallRecordingReadyEventRecordingTypeEnum = {
  COMPOSITE: "composite",
  INDIVIDUAL: "individual",
  RAW: "raw"
};
var CallRecordingStartedEventRecordingTypeEnum = {
  COMPOSITE: "composite",
  INDIVIDUAL: "individual",
  RAW: "raw"
};
var CallRecordingStoppedEventRecordingTypeEnum = {
  COMPOSITE: "composite",
  INDIVIDUAL: "individual",
  RAW: "raw"
};
...
var IndividualRecordingSettingsRequestModeEnum = {
  AVAILABLE: "available",
  DISABLED: "disabled",
  AUTO_ON: "auto-on"
};
var IndividualRecordingSettingsResponseModeEnum = {
  AVAILABLE: "available",
  DISABLED: "disabled",
  AUTO_ON: "auto-on"
};
...
var RawRecordingSettingsRequestModeEnum = {
  AVAILABLE: "available",
  DISABLED: "disabled",
  AUTO_ON: "auto-on"
};
var RawRecordingSettingsResponseModeEnum = {
  AVAILABLE: "available",
  DISABLED: "disabled",
  AUTO_ON: "auto-on"
};
...
var TIMEOUT_SYMBOL = "@@stream-io/timeout";
var withTimeout = (timeoutMs, trace) => {
  const scheduleTimeout = (methodName) => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => {
      trace == null ? void 0 : trace(`${methodName}Timeout`, [timeoutMs]);
...
var withRequestLogger = (logger, level = "trace") => {
  return {
    interceptUnary: (next, method, input, options) => {
...
var withRequestTracer = (trace) => {
  const exclusions = /* @__PURE__ */ new Set(["SendStats"]);
  const responseInclusions = /* @__PURE__ */ new Set(["SetPublisher"]);
...
var retryable = async (rpc, signal, maxRetries = Number.POSITIVE_INFINITY) => {
  var _a;
  let attempt = 0;
  let result = void 0;
  do {
    ...
    try {
      ...
      const isRequestCancelled = err instanceof RpcError && err.message !== TIMEOUT_SYMBOL && err.code === TwirpErrorCode[TwirpErrorCode.cancelled];
...
var setStartBitrate = (offerSdp, maxBitrateKbps, startBitrateFactor, targetMid) => {
  const startBitrate = Math.min(maxBitrateKbps, Math.max(300, startBitrateFactor * maxBitrateKbps));
  const parsedSdp = (0, import_sdp_transform.parse)(offerSdp);
  const targetCodecs = /* @__PURE__ */ new Set(["av1", "vp9", "h264"]);
  for (const media of parsedSdp.media) {
    ...
    for (const rtp of media.rtp) {
      ...
      if (existingFmtp) {
        const config2 = existingFmtp.config ?? "";
        if (!config2.includes("x-google-start-bitrate")) {
          existingFmtp.config = config2 ? `${config2};x-google-start-bitrate=${startBitrate}` : `x-google-start-bitrate=${startBitrate}`;
        }
      } else {
        fmtpList.push({
          payload: rtp.payload,
  ...
  return (0, import_sdp_transform.write)(parsedSdp);
};
...
var removeCodecsExcept = (sdp2, codecMimeTypeToKeep, fmtpProfileToKeep) => {
  ...
  for (const media of parsed.media) {
    ...
    media.rtcpFb = (_a = media.rtcpFb) == null ? void 0 : _a.filter((fb) => typeof fb.payload === "number" ? payloadsToKeep.has(fb.payload) : true);
    const payloads = [];
...
var Dispatcher = class {
  constructor() {
    ...
    this.getHandlers = (eventName) => {
      ...
      if (existing)
        return existing;
      const next = { byTag: /* @__PURE__ */ new Map(), dynamic: [] };
...
var hasAudio$1 = (p) => p.publishedTracks.includes(TrackType.AUDIO);
...
var livestreamOrAudioRoomSortPreset = combineComparators(ifInvisibleOrUnknownBy(combineComparators(dominantSpeaker, speaking, reactionType("raised-hand"), withVideoIngressSource, publishingVideo, publishingAudio)), role("admin", "host", "speaker"));
...
var CallState = class {
  ...
  constructor() {
    ...
    this.updateOrAddParticipant = (sessionId, participant, patch2) => {
      return this.setParticipants((participants) => {
        ...
        const nextParticipants = participants.map((p) => {
          if (p.sessionId === sessionId) {
            ...
            const thePatch = typeof patch2 === "function" ? patch2(updated) : patch2;
    ...
    this.updateFromCallResponse = (call) => {
      ...
      this.setCurrentValue(this.recordingSubject, call.recording || (composite_recording == null ? void 0 : composite_recording.status) === "running");
      this.setCurrentValue(this.individualRecordingSubject, (individual_recording == null ? void 0 : individual_recording.status) === "running");
      this.setCurrentValue(this.rawRecordingSubject, (raw_recording == null ? void 0 : raw_recording.status) === "running");
...
var isSupportedBrowser = async () => {
  ...
  const [major2] = browser.version.split(".");
  const version2 = parseInt(major2, 10);
...
var getCameraStats = (mediaSource) => ({
...
var SfuStatsReporter = class {
  constructor(sfuClient, { options, clientDetails, subscriber, publisher, microphone, camera, state, tracer, unifiedSessionId }) {
    ...
    this.scheduleNextReport = () => {
      const intervals = [1500, 3e3, 3e3, 5e3];
  ...
  constructor(peerType, { sfuClient, connectionConfig, state, dispatcher, onReconnectionNeeded, tag, enableTracing, clientPublishOptions, iceRestartDelay = 2500 }) {
    ...
    this.tryRestartIce = () => {
      this.restartIce().catch((e) => {
        ...
        (_a = this.onReconnectionNeeded) == null ? void 0 : _a.call(this, strategy, reason, this.peerType);
      });
    };
    this.on = (event, fn) => {
    ...
    this.onConnectionStateChange = async () => {
      ...
      if (state2 === "failed") {
        (_a = this.onReconnectionNeeded) == null ? void 0 : _a.call(this, WebsocketReconnectStrategy.REJOIN, "Connection failed", this.peerType);
        return;
      }
...
var TransceiverCache = class {
  constructor() {
    ...
    this.update = (publishOption, patch2) => {
      const bundle = this.get(publishOption);
      if (bundle)
        Object.assign(bundle, patch2);
    };
    ...
    this.negotiate = async (options) => {
      return withoutConcurrency(`publisher.negotiate.${this.lock}`, async () => {
        ...
        try {
          ...
          let sdp2 = dangerouslyForceCodec ? removeCodecsExcept(baseSdp, dangerouslyForceCodec, fmtpLine) : baseSdp;
          if (dangerouslySetStartBitrateFactor !== void 0) {
            this.transceiverCache.items().forEach((t2) => {
              var _a, _b;
              if (t2.publishOption.trackType !== TrackType.VIDEO)
                return;
              const maxBitrateBps = t2.publishOption.bitrate;
              const trackId = (_a = t2.transceiver.sender.track) == null ? void 0 : _a.id;
              if (!trackId)
                return;
              const mid = (_b = tracks.find((x) => x.trackId === trackId)) == null ? void 0 : _b.mid;
              if (!mid)
                return;
              sdp2 = setStartBitrate(
                sdp2,
                maxBitrateBps / 1e3,
                // convert to kbps
...
var SfuJoinError = class extends Error {
  constructor(event) {
    var _a;
    super(((_a = event.error) == null ? void 0 : _a.message) || "Join Error");
    this.errorEvent = event;
    this.unrecoverable = event.reconnectStrategy === WebsocketReconnectStrategy.DISCONNECT;
  }
  static isJoinErrorCode(event) {
    var _a;
    const code = (_a = event.error) == null ? void 0 : _a.code;
    return code === ErrorCode.SFU_FULL || code === ErrorCode.SFU_SHUTTING_DOWN || code === ErrorCode.CALL_PARTICIPANT_LIMIT_REACHED;
  }
};
var StreamSfuClient = class _StreamSfuClient {
   ...
   */
  constructor({ dispatcher, credentials, sessionId, cid, tag, joinResponseTimeout = 5e3, rpcRequestTimeout = 5e3, onSignalClose, streamClient, enableTracing }) {
    var _a, _b;
    ...
    this.join = async (data) => {
      ...
      let unsubscribeJoinResponse = void 0;
      let unsubscribeJoinErrorEvents = void 0;
      const cleanupJoinSubscriptions = () => {
        clearTimeout(timeoutId);
        timeoutId = void 0;
        unsubscribeJoinErrorEvents == null ? void 0 : unsubscribeJoinErrorEvents();
        unsubscribeJoinErrorEvents = void 0;
        unsubscribeJoinResponse == null ? void 0 : unsubscribeJoinResponse();
        unsubscribeJoinResponse = void 0;
      };
    ...
    this.rpc = createSignalClient({
      ...
      interceptors: [
        ...
        withTimeout(rpcRequestTimeout, (_a = this.tracer) == null ? void 0 : _a.trace)
      ].filter((v) => !!v)
    });
    this.unsubscribeIceTrickle = dispatcher.on("iceTrickle", tag, (t2) => {
      this.iceTrickleBuffer.push(t2);
    });
    ...
    this.createWebSocket({
      attempt: tag,
      user_id: ((_b = streamClient.user) == null ? void 0 : _b.id) || "",
  ...
  return function onTrackUnpublished(e) {
    ...
    if (e.participant) {
      ...
      state.updateOrAddParticipant(sessionId, participant, (p) => {
        var _a;
        return {
          pausedTracks: (_a = p.pausedTracks) == null ? void 0 : _a.filter((t2) => t2 !== type)
        };
      });
    } else {
      state.updateParticipant(sessionId, (p) => {
...
var DynascaleManager = class {
  ...
  constructor(callState, speaker, tracer) {
    ...
    this.getOrCreateAudioContext = () => {
      ...
      this.tracer.trace("audioContext.create", context2.state);
      if (context2.state === "suspended") {
        document.addEventListener("click", this.resumeAudioContext);
      }
      context2.addEventListener("statechange", () => {
        this.tracer.trace("audioContext.state", context2.state);
        if (context2.state === "interrupted") {
          this.resumeAudioContext();
        }
      });
    ...
    this.resumeAudioContext = () => {
      ...
      if (state === "suspended" || state === "interrupted") {
        ...
        this.audioContext.resume().then(() => {
          var _a;
          this.tracer.trace(tag, (_a = this.audioContext) == null ? void 0 : _a.state);
          document.removeEventListener("click", this.resumeAudioContext);
        }, (err) => {
          var _a;
          this.tracer.trace(`${tag}Error`, (_a = this.audioContext) == null ? void 0 : _a.state);
...
var defaultDeviceId = "default";
var isLocalStorageAvailable = () => typeof window !== "undefined" && typeof window.localStorage !== "undefined";
var normalize = (options) => {
  return {
    storageKey: (options == null ? void 0 : options.storageKey) ?? `@stream-io/device-preferences`,
    enabled: isLocalStorageAvailable() && !isReactNative2() ? (options == null ? void 0 : options.enabled) ?? true : false
  };
};
var createSyntheticDevice = (deviceId, kind) => {
  return { deviceId, kind, label: "", groupId: "" };
};
var readPreferences = (storageKey) => {
  try {
...
var writePreferences = (currentDevice, deviceKey, muted, storageKey) => {
  if (!isLocalStorageAvailable())
    return;
  const selectedDeviceId = (currentDevice == null ? void 0 : currentDevice.deviceId) ?? defaultDeviceId;
  const selectedDeviceLabel = (currentDevice == null ? void 0 : currentDevice.label) ?? "";
  const preferences = readPreferences(storageKey);
  const preferenceHistory = [preferences[deviceKey] ?? []].flat().filter((p) => p.selectedDeviceId !== selectedDeviceId && (p.selectedDeviceLabel === "" || p.selectedDeviceLabel !== selectedDeviceLabel));
...
var toPreferenceList = (preference) => preference ? [preference].flat() : [];
var DeviceManager = class {
  ...
  persistPreference(selectedDevice, status) {
    ...
    const muted = status === "disabled" ? true : status === "enabled" ? false : void 0;
    const { storageKey } = this.devicePersistence;
    if (!selectedDevice) {
      writePreferences(void 0, deviceKey, muted, storageKey);
      return;
    }
    const devices = getCurrentValue(this.listDevices()) || [];
    const currentDevice = this.findDevice(devices, selectedDevice) ?? createSyntheticDevice(selectedDevice, deviceKind);
  ...
  async applyPersistedPreferences(enabledInCallType) {
    ...
    for (const preference of preferenceList) {
      ...
      if (preference.selectedDeviceId === defaultDeviceId)
        break;
      const device = devices.find((d) => d.deviceId === preference.selectedDeviceId) ?? devices.find((d) => d.label === preference.selectedDeviceLabel);
      if (device) {
        appliedDevice = true;
...
var CameraManager = class extends DeviceManager {
  ...
  async selectDirection(direction, options = {}) {
    ...
    if (isReactNative2() || !enableCamera)
      return;
  ...
  async apply(settings, publish2) {
    ...
    const shouldApplyDefaults = this.state.status === void 0 && this.state.optimisticStatus === void 0;
    let persistedPreferencesApplied = false;
    if (shouldApplyDefaults && this.devicePersistence.enabled) {
      persistedPreferencesApplied = await this.applyPersistedPreferences(enabledInCallType);
    }
    const canPublish = this.call.permissionsContext.canPublish(this.trackType);
    ...
    if (canPublish && publish2 && this.enabled && mediaStream) {
...
var hasAudio = (analyser) => {
  ...
  let squareSum = 0;
  for (const sample2 of data) {
    const centered = sample2 - 128;
...
var noEmit = (nextState) => ({
  shouldEmit: false,
  nextState
});
var emit = (capturesAudio, nextState) => ({ shouldEmit: true, nextState, capturesAudio });
var transitionState = (state, audioDetected, options) => {
  if (audioDetected) {
  ...
  switch (state.kind) {
    ...
    case "DETECTING": {
      ...
      return elapsed >= noAudioThresholdMs ? emit(false, { kind: "EMITTING", noAudioStartTime, lastEmitTime: now }) : noEmit(state);
    }
    case "EMITTING": {
      const timeSinceLastEmit = now - state.lastEmitTime;
      return timeSinceLastEmit >= emitIntervalMs ? emit(false, { ...state, lastEmitTime: now }) : noEmit(state);
    }
  }
};
var createAudioAnalyzer = (audioStream, fftSize) => {
  const audioContext = new AudioContext();
  const analyser = audioContext.createAnalyser();
  analyser.fftSize = fftSize;
  const microphone = audioContext.createMediaStreamSource(audioStream);
...
var createNoAudioDetector = (audioStream, options) => {
  ...
  const detectionIntervalId = setInterval(() => {
    ...
    const audioDetected = (track == null ? void 0 : track.readyState) === "live" && hasAudio(analyser);
...
var RNSpeechDetector = class {
  ...
  async start(onSoundDetectedStateChanged) {
    try {
      const audioStream = this.externalAudioStream != null ? this.externalAudioStream : await navigator.mediaDevices.getUserMedia({ audio: true });
...
var MicrophoneManager = class extends AudioDeviceManager {
    ...
    this.subscriptions.push(createSubscription(this.call.state.callingState$, (callingState) => {
      ...
      if (autoOn && callingState === CallingState.JOINED) {
        this.noiseCancellationRegistration.then(() => {
          ...
          var _a2;
          if (canAutoEnable) {
            (_a2 = this.noiseCancellation) == null ? void 0 : _a2.enable().catch((err) => {
    ...
    if (!isReactNative2()) {
      const unsubscribe = createSafeAsyncSubscription(combineLatest([this.state.status$, this.state.mediaStream$]), async ([status, mediaStream]) => {
        ...
        if (this.noAudioDetectorCleanup) {
          ...
          this.noAudioDetectorCleanup = void 0;
        ...
        const devices = await firstValueFrom(this.listDevices());
        const label = (_a = devices.find((d) => d.deviceId === deviceId)) == null ? void 0 : _a.label;
  ...
  async performTest(deviceId, options) {
    ...
    const { testDurationMs = 3e3 } = options || {};
  ...
  async teardownSpeakingWhileMutedDetection() {
    ...
    this.soundDetectorDeviceId = void 0;
  ...
  async hasPermission(permissionState) {
    ...
    const nativePermissions = (_a = globalThis.streamRNVideoSDK) == null ? void 0 : _a.permissions;
...
var SpeakerManager = class {
  ...
  apply(settings) {
    return isReactNative2() ? this.applyRN(settings) : this.applyWeb();
  }
  applyWeb() {
    ...
    const nextDeviceId = preference.selectedDeviceId === defaultDeviceId ? "" : preference.selectedDeviceId;
  ...
  applyRN(settings) {
    var _a;
    const speakerOnWithSettingsPriority = settings.video.camera_default_on || settings.audio.speaker_default_on || settings.audio.default_device === AudioSettingsRequestDefaultDeviceEnum.SPEAKER;
    const defaultDevice = speakerOnWithSettingsPriority ? AudioSettingsRequestDefaultDeviceEnum.SPEAKER : AudioSettingsRequestDefaultDeviceEnum.EARPIECE;
    if (this.defaultDevice !== defaultDevice) {
      this.call.logger.debug("SpeakerManager: setting default device", {
        defaultDevice
      });
      this.defaultDevice = defaultDevice;
      (_a = globalThis.streamRNVideoSDK) == null ? void 0 : _a.callManager.setup({
        defaultDevice
      });
    }
  }
  setup() {
    ...
    if (!isReactNative2() && this.devicePersistence.enabled) {
  ...
  persistSpeakerDevicePreference(selectedDevice) {
    const { storageKey } = this.devicePersistence;
    const devices = getCurrentValue(this.listDevices()) || [];
    const currentDevice = devices.find((d) => d.deviceId === selectedDevice) ?? createSyntheticDevice(selectedDevice, "audiooutput");
    writePreferences(currentDevice, "speaker", void 0, storageKey);
  }
};
var assertUnsupportedInReactNative = () => {
  if (isReactNative2()) {
  ...
  constructor({ type, id, streamClient, members, ownCapabilities, sortParticipantsBy, clientStore, ringing = false, watching = false }) {
    ...
    this.leave = async ({ reject, reason, message } = {}) => {
      ...
      await withoutConcurrency(this.joinLeaveConcurrencyTag, async () => {
        ...
        await ((_f = this.sfuClient) == null ? void 0 : _f.leaveAndClose(leaveReason));
        ...
        (_g = globalThis.streamRNVideoSDK) == null ? void 0 : _g.callManager.stop();
        this.camera.dispose();
    ...
    this.updateFromRingingEvent = async (event) => {
      ...
      const skipSpeakerApply = isReactNative2();
    ...
    this.get = async (params) => {
      ...
      const skipSpeakerApply = isReactNative2() ? (params == null ? void 0 : params.ring) === true ? true : this.ringing : false;
    ...
    this.getOrCreate = async (data) => {
      ...
      const skipSpeakerApply = isReactNative2() ? (data == null ? void 0 : data.ring) === true ? true : this.ringing : false;
      await this.applyDeviceConfig(response.call.settings, false, skipSpeakerApply);
      return response;
    };
    ...
    this.join = async ({ maxJoinRetries = 3, joinResponseTimeout, rpcRequestTimeout, ...data } = {}) => {
      ...
      for (let attempt = 0; attempt < maxJoinRetries; attempt++) {
        try {
          ...
          if (err instanceof ErrorFromResponse && err.unrecoverable || err instanceof SfuJoinError && err.unrecoverable) {
            throw err;
          }
          const switchSfu = err instanceof SfuJoinError && SfuJoinError.isJoinErrorCode(err.errorEvent);
    ...
    this.doJoin = async (data) => {
      ...
      if (!this.deviceSettingsAppliedOnce && this.state.settings) {
        ...
        (_c = globalThis.streamRNVideoSDK) == null ? void 0 : _c.callManager.start();
        this.deviceSettingsAppliedOnce = true;
      }
      (_d = this.joinCallData) == null ? true : delete _d.ring;
      (_e = this.joinCallData) == null ? true : delete _e.notify;
      this.reconnectStrategy = WebsocketReconnectStrategy.UNSPECIFIED;
      this.reconnectReason = "";
    ...
    this.reconnectMigrate = async () => {
      ...
      try {
        ...
        (_b = this.joinCallData) == null ? true : delete _b.migrating_from_list;
      }
      await this.restorePublishedTracks();
      this.restoreSubscribedTracks();
      try {
      ...
      (_c = this.sfuStatsReporter) == null ? void 0 : _c.sendReconnectionTime(WebsocketReconnectStrategy.MIGRATE, (Date.now() - reconnectStartTime) / 1e3);
    };
    this.registerReconnectHandlers = () => {
    ...
    this.startRecording = async (dataOrType, type2) => {
      type2 = typeof dataOrType === "string" ? dataOrType : type2;
      dataOrType = typeof dataOrType === "string" ? void 0 : dataOrType;
      const endpoint = !type2 ? `/start_recording` : `/recordings/${encodeURIComponent(type2)}/start`;
      return this.streamClient.post(`${this.streamClientBasePath}${endpoint}`, dataOrType);
    };
    this.stopRecording = async (type2) => {
      const endpoint = !type2 ? `/stop_recording` : `/recordings/${encodeURIComponent(type2)}/stop`;
    ...
    this.getCallStatsMap = async (params = {}, callSessionId = ((_a) => (_a = this.state.session) == null ? void 0 : _a.id)()) => {
      if (!callSessionId)
    ...
    this.applyDeviceConfig = async (settings, publish2, skipSpeakerApply) => {
      if (!skipSpeakerApply) {
  ...
  constructor(apiKeyOrArgs, opts) {
    ...
    this.queryCalls = async (data = {}) => {
      ...
      for (const c of response.calls) {
        ...
        await call.applyDeviceConfig(c.call.settings, false, isReactNative2());
        if (data.watch) {
          await call.setup();
          this.writeableStateStore.registerCall(call);
        }
    ...
    this.shouldRejectCall = (currentCallId) => {
      if (!this.rejectCallWhenBusy)
        return false;
      return this.state.calls.some((c) => c.cid !== currentCallId && c.ringing && c.state.callingState !== CallingState.IDLE && c.state.callingState !== CallingState.LEFT && c.state.callingState !== CallingState.RECONNECTING_FAILED);
    };
...
var setPath = (object, path, newValue) => {
  ...
  while (last3.obj === void 0 && p.length) {
    ...
    if ((last3 == null ? void 0 : last3.obj) && typeof last3.obj[`${last3.k}.${e}`] !== "undefined") {
      last3.obj = void 0;
    }
  }
...
var Translator = class _Translator extends EventEmitter {
  ...
  exists(key, o = {
    ...
    const isObject2 = shouldHandleAsObject(resolved2.res);
...
var mapToRegistry = (translationsMap, namespace) => Object.entries(translationsMap).reduce((acc, [lng, translations]) => {
...
var useIsCallRawRecordingInProgress = () => {
  const { rawRecording$ } = useCallState();
  return useObservableValue(rawRecording$);
};
var useIsCallIndividualRecordingInProgress = () => {
...
//# sourceMappingURL=chunk-GIZTFSNM.js.map
